...
from .tesla_custom_lib.controller import Controller
...
API_SCHEMA = vol.Schema(
    {
        vol.Optional("base_url"): vol.All(cv.string, vol.Length(min=1)),
...
SCAN_INTERVAL_SCHEMA = vol.Schema(
    {
        vol.Optional(CONF_BASE_URL): vol.All(cv.string, vol.Length(min=1)),
...
def async_setup_services(hass) -> None:
    ...
    async def api(call):
        ...
        base_url = service_data.get(CONF_BASE_URL, "")

        if len(hass.config_entries.async_entries(DOMAIN)) > 1 and not base_url:
            raise ValueError("BaseURL address missing")
        controller: Controller = None
        for entry in hass.config_entries.async_entries(DOMAIN):
            if (
                len(hass.config_entries.async_entries(DOMAIN)) > 1
                and entry.title != base_url
            ):
                continue
            entry_data = hass.data[DOMAIN][entry.entry_id]
            controller = entry_data["controller"]
        if controller is None:
            raise ValueError(f"No Tesla controllers found for baseURL {base_url}")
        command = call.data.get(ATTR_COMMAND)
        parameters: dict = call.data.get(ATTR_PARAMETERS, {})
        _LOGGER.debug(
            "Service api called with baseURL: %s command: %s parameters: %s",
            base_url,
            command,
            parameters,
        )
    ...
    async def set_update_interval(call):
        ...
        for entry in hass.config_entries.async_entries(DOMAIN):
            ...
            controller = entry_data["controller"]
        if controller is None:
            raise ValueError(f"No Tesla controllers found for email {base_url}")
